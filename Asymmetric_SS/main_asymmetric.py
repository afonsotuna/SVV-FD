...
def num_model_asym_reference(output=1, t_lookup=3717, t_limit=14, dutch_roll=True, block_fuel=4050, passenger_weight=695, b=15.911,
    ...
    if output == 1:
        for i in range(n_points):
            ...
            data_event[i, 1] = flight_data[index + i, 20] - flight_data[index, 20]  # Output phi
    elif output == 2:
        for i in range(n_points):
            data_event[i, 0] = flight_data[index + i, 47] - flight_data[index, 47]
            data_event[i, 1] = flight_data[index + i, 25] - flight_data[index, 25]# Output pb/2V
    elif output == 3:
        for i in range(n_points):
            data_event[i, 0] = flight_data[index + i, 47] - flight_data[index, 47]
            data_event[i, 1] = flight_data[index + i, 27] - flight_data[index, 27]# Output rb/2V
    t1 = data_event[:, 0]
...
def make_plot_asym(output=1, dutch_roll=True, t_lookup=3717, t_limit=14, block_fuel=4050, passenger_weight=695, b=15.911, CY_b=-0.7500,
    ...
    plt.plot(t1, y1, label='Reference data - rb/2V')
    plt.plot(t2, y2, label='System response - rb/2V')
