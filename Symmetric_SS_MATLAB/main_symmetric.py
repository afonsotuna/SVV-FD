...
for i in range(n_points):
    ...
    data_event[i, 1] = flight_data[index + i, 26]  # Outputs: ?? - u / 0 - alpha / 21 - theta / 26 - q
t1 = data_event[:, 0]
...
y2 = out[3, :] + initial_cond[3]  # Outputs: 0 - u / 1 - alpha / 2 - theta / 3 - q
...
plt.legend()
plt.xlabel('Reference data vs system response between ' + str(t_lookup) + ' [s] and ' + str(t_interval) + ' [s] ' + motion + '.')
